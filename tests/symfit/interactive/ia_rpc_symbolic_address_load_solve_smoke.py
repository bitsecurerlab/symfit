...
def main():
    ...
    try:
        ...
        if root["label"].lower() != newest_label.lower():
            ...
            return 1
        if root.get("taken") is not True:
            print(f"Expected root path constraint to report taken=true, got {root.get('taken')}", file=sys.stderr)
            return 1
        if root["op"] != "ICmp":
        ...
        if summary["path_constraints"]["count"] < 1:
            ...
            return 1
        if any(entry.get("taken") is not True for entry in nested):
            print(f"Expected nested path constraints to report taken=true, got {nested}", file=sys.stderr)
            return 1
