...
class QemuUserInstrumentedBackend:
    ...
    def start(
        ...
        qemu_mode = self._qemu_mode(qemu_config)
        _debug_log(f"start target={target!r} args={args!r} cwd={cwd!r} mode={qemu_mode} launch={bool(qemu_config.get('launch'))}")
        self._capabilities = self._default_capabilities()
        try:
            ...
            if qemu_config.get("launch"):
                if self._process_runner is None:
                    self._process_runner = QemuSystemProcessRunner() if qemu_mode == "system" else QemuUserProcessRunner()
                if qemu_mode == "system":
            ...
            if self._process_runner is not None and self._process_runner.config is not None:
                ...
                launched_qemu_user_path = getattr(config, "qemu_user_path", None)
                launched_qemu_system_path = getattr(config, "qemu_system_path", None)
                launched_qemu_path = launched_qemu_user_path or launched_qemu_system_path
            self._state.update(
                {
                    "session_status": "idle",
                    "qemu_mode": qemu_mode,
    ...
    @staticmethod
    def _qemu_mode(qemu_config: dict[str, Any]) -> str:
        raw = str(qemu_config.get("mode") or ("system" if qemu_config.get("qemu_system_path") else "user")).strip().lower()
        if raw in {"system", "softmmu", "qemu-system"}:
            return "system"
        return "user"
    ...
    def read_memory(self, address: str, size: int, address_space: str | None = None) -> dict[str, Any]:
        ...
        # Old address space assignment code left commented out for potential rollback if necessary
        '''
        if address_space:
